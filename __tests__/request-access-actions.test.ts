...
describe('signUpForAccess', () => {
  test('creates the account with the name as metadata, email lowercased', async () => {
    const r = await signUpForAccess(null, form({ full_name: ' Nina New ', email: 'Nina@QWealth.com.au', password: 'a-long-enough-password' }))
    expect(r && 'ok' in r).toBe(true)
    expect(log).toEqual([
      { call: 'signUp', args: [{ email: 'nina@qwealth.com.au', password: 'a-long-enough-password', options: { data: { full_name: 'Nina New' } } }] },
    ])
    expect(r && 'message' in r ? r.message : '').toContain('nina@qwealth.com.au')
  })

  test('a short password never reaches Auth', async () => {
    const r = await signUpForAccess(null, form({ full_name: 'N', email: 'n@qwealth.com.au', password: 'short' }))
    expect(r).toEqual({ error: 'Use a password of at least 12 characters.' })
    expect(log).toEqual([])
  })

  test('a missing field never reaches Auth', async () => {
    const r = await signUpForAccess(null, form({ email: 'n@qwealth.com.au', password: 'a-long-enough-password' }))
  ...
  test('Auth’s own refusal comes back as it was said', async () => {
    ...
    const r = await signUpForAccess(null, form({ full_name: 'N', email: 'n@qwealth.com.au', password: 'a-long-enough-password' }))
...
describe('requestStaffAccess', () => {
  test('asks the database, then returns to the request page', async () => {
    await expect(requestStaffAccess(null, form({ full_name: ' Nina New ' }))).rejects.toThrow('redirect:/request-access')
    expect(log).toEqual([{ call: 'rpc:request_staff_access', args: [{ p_full_name: 'Nina New' }] }])
  })

  test('goes back where the person came from when that is on this site, and nowhere else', async () => {
    await expect(requestStaffAccess(null, form({ full_name: 'N', next: '/oauth/consent?authorization_id=a' }))).rejects.toThrow(
      'redirect:/oauth/consent?authorization_id=a',
    )
    await expect(requestStaffAccess(null, form({ full_name: 'N', next: 'https://evil.example/' }))).rejects.toThrow('redirect:/request-access')
    await expect(requestStaffAccess(null, form({ full_name: 'N', next: '//evil.example' }))).rejects.toThrow('redirect:/request-access')
  })

  test('the database’s sentence is the answer', async () => {
    rpcError = 'Access requests are limited to Q Wealth staff email addresses'
    const r = await requestStaffAccess(null, form({ full_name: 'N' }))
  ...
  test('a blank name never reaches the database', async () => {
    const r = await requestStaffAccess(null, form({ full_name: '   ' }))
    expect(r).toEqual({ error: 'Enter your full name.' })
    expect(log).toEqual([])
  })
})
